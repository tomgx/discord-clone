...
const Channelbar = () => {
    ...
    return (
        <div className='select-none rounded-l relative left-[60px] w-[220px] h-screen bg-[#282b30]'>
            <div className='cursor-pointer h-[40px] w-[220px] shadow rounded-l' onClick={handleClick} style={{ backgroundColor: toggle ? '#282b30' : '#6d6d6d2d' }}>
                <h1 className='font-bold text-[#eee] py-2 pl-4 hover:bg-[#6d6d6d2d] rounded-l'>
                    More Information
                </h1>
                {toggle ?
                    <IoIosArrowDown className='absolute top-3 right-5' style={{ color: '#eee' }} /> :
                    <IoMdClose className='absolute top-3 right-5' style={{ color: '#eee' }} />
                }
